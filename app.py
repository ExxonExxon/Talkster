...
def profile():
    ...
    if request.method == 'POST':
        ...
        if 'file' in request.files:
            ...
            if file and allowed_file(file.filename):
                ...
                file.save(filepath)
                # Update the user's profile picture in the database if needed
                connection = get_db_connection()
                cursor = connection.cursor()
        ...
        if new_username:
            ...
            connection.commit()
            # Update the user's username in the session
    ...
    # Retrieve user data for rendering the template
    profile_pic = session.get('profile_pic')
    current_username = session.get('username')

    return render_template('profile.html', profile_pic=profile_pic, current_username=current_username)
...
def update_username():
    ...
    if new_username:
        ...
        # Update the username in the messages table
        cursor.execute('UPDATE messages SET username = ? WHERE username = ?', (new_username, session['username']))
